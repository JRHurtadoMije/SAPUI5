sap.ui.define([
    ...
    return Controller.extend("com.jr.jrhub.controller.Graficos", {
        ...
        _applyMetricFeeds: function () {
            const sMetric = this.getView().getModel("view").getProperty("/filters/metric");
            const sMeasureName = (sMetric === "sales") ? "Ventas" : "Unidades";
            ...
            const sFmt = (sMetric === "sales") ? "Currency" : "#,##0"; // usamos el formato legible para unidades
        ...
        _applyAllFilters: function () {
            ...
            const sCat = oVM.getProperty("/filters/category");
            const iMin = Number(oVM.getProperty("/filters/minPrice")) || 0;
        ...
        _filterDataset: function (oViz, aFilters) {
            if (!oViz) { return; }
            const oDs = oViz.getDataset();
            const oBinding = oDs && oDs.getBinding("data");
            if (oBinding) { oBinding.filter(aFilters); }
        },

        _applyResponsiveVizProps: function () {
            const range = sap.ui.Device.media.getCurrentRange("StdExt"); // S, M, L, XL
            const bSmall = (range.name === "Phone" || range.name === "Tablet");
            const aCharts = [this.byId("vizBar"), this.byId("vizDonut"), this.byId("vizLine")];
